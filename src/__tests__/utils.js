...
test('resolveKcdScripts resolves to src/index.js when in the jmd-scripts package', () => {
  mockPkg({pkg: {name: 'jmd-scripts'}})
  expect(require('../utils').resolveKcdScripts()).toBe(
    require.resolve('../').replace(process.cwd(), '.'),
  )
})

test('resolveKcdScripts resolves to jmd-scripts if not in the jmd-scripts package', () => {
  mockPkg({pkg: {name: 'not-jmd-scripts'}})
  whichSyncMock.mockImplementationOnce(() => require.resolve('../'))
  expect(require('../utils').resolveKcdScripts()).toBe('jmd-scripts')
})
